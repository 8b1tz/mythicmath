...
def _error_code_value(code: ErrorCodeValue) -> str:
...
class ApiError(HTTPException):
    def __init__(
        ...
        self.code = _error_code_value(code)


def bad_request(code: ErrorCodeValue, detail: str) -> ApiError:
    return ApiError(status.HTTP_400_BAD_REQUEST, code=code, detail=detail)


def unauthorized(code: ErrorCodeValue, detail: str) -> ApiError:
    return ApiError(status.HTTP_401_UNAUTHORIZED, code=code, detail=detail)


def forbidden(code: ErrorCodeValue, detail: str) -> ApiError:
    return ApiError(status.HTTP_403_FORBIDDEN, code=code, detail=detail)


def conflict(code: ErrorCodeValue, detail: str) -> ApiError:
    return ApiError(status.HTTP_409_CONFLICT, code=code, detail=detail)


def unprocessable_entity(code: ErrorCodeValue, detail: str) -> ApiError:
    return ApiError(status.HTTP_422_UNPROCESSABLE_ENTITY, code=code, detail=detail)


def internal_server_error(code: ErrorCodeValue, detail: str) -> ApiError:
...
def _default_error_code(status_code: int) -> str:
    ...
    return _error_code_value(fallback_codes.get(status_code, f"HTTP_{status_code}"))
